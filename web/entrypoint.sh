...
if [ ! -f "/certs/haproxy/cert.pem" ]; then
  ...
  cp /etc/ssl/certs/key.pem /certs/key.pem
  cp /etc/ssl/certs/ca.pem /certs/ca.pem

  mkdir /certs/haproxy/

  cp /etc/ssl/certs/key.pem /certs/haproxy/cert.pem
  cat /etc/ssl/certs/ca.pem >> /certs/haproxy/cert.pem
else
